...
namespace HolePlugin
{
    ...
    public class HolePlugin : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            View3D view3D = new FilteredElementCollector(arDoc) //ищем есть ли у нас 3D вид, т.к. метод поиска пересечения работает только на них
                .OfClass(typeof(View3D))
                .OfType<View3D>()
                .Where(x => !x.IsTemplate) //отсекаем шаблоны 3D видов
                .FirstOrDefault();
            if (view3D == null)
            {
            ...
            foreach (Duct d in ducts) //для каждого воздуховода из коллекции
            {
                ...
                foreach (ReferenceWithContext r in intersections)
                {
                    ...
                    Wall wall = arDoc.GetElement(reference.ElementId) as Wall; //берем стену которую пересекает
    ...
    
    {
        public bool Equals(ReferenceWithContext x, ReferenceWithContext y)
        {
            ...
            return xReference.LinkedElementId == yReference.LinkedElementId
                       && xReference.ElementId == yReference.ElementId; //определяет, если у обоих элементов одинаковый ElementID (точки на одной стене), то возвращает true
        }
